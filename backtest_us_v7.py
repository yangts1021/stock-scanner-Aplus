"""
美股回測 v7（與 backtest_v7.py 結構對齊，僅改資料來源與大盤指數）
- 回檔範圍 10~30%
- 大盤多頭過濾：訊號日當天 ^IXIC 收盤 > 60MA 且 60MA 20 日斜率 > 0
- 隔日開盤進場 / R:R≥1.0 / 前低停損 / 過前高停利 / 20 天追蹤
"""
import sys, os, time
import pandas as pd
import numpy as np
...
HOLD_DAYS      = 20
MIN_RR         = 1.0
MIN_RISK_PCT   = 5.0   # 停損距離 < 5% 視為過窄假訊號，過濾掉
LOOKBACK       = 60
DEDUP_COOLDOWN = 30

OUT_SIGNALS = "/Users/rick/Developer/Aplus/backtest_us_v7_signals.csv"
OUT_SUMMARY = "/Users/rick/Developer/Aplus/backtest_us_v7_summary.txt"


def load_ndx_filter(con):
...
def simulate_one(df, ticker, name, sidx):
    ...
    if risk_pct < MIN_RISK_PCT:
        return {**base, 'outcome': 'TIGHT_STOP', 'exit_date': '', 'exit_price': None,
                'return_pct': 0.0, 'days_held': 0,
                'max_gain_pct': 0.0, 'max_drawdown_pct': 0.0}
...
def run():
    con = get_conn()
    bull = load_ndx_filter(con)
    n_bull = bull.sum(); n_idx = len(bull)
    print(f"^IXIC 多頭日數 {n_bull}/{n_idx} ({n_bull/n_idx*100:.1f}%)")
...
def summarize(df, n_filtered_bear):
    ...
    n_tight = (df['outcome']=='TIGHT_STOP').sum()
    ...
    lines = []
    lines.append("="*64)
    lines.append(" 美股回測 v7：回檔10~30% + 大盤多頭 + R:R≥{} + 前低停損 + 過前高停利".format(MIN_RR))
    lines.append("="*64)
    if n_trades:
    ...
    lines.append(f"  停損<{MIN_RISK_PCT}% skip:  {n_tight}  ({n_tight/n_total*100:.1f}%)")
